...
// 需要完成的任务列表
var TASK_LIST = ["每日", "AR", "浏览领", "浏览并关注8s", "浏览8秒", "浏览8s", "累计浏览", "浏览并关注可得2000", "浏览可得", "入会", "浏览并关注可得", "去首页浮层进入", "浏览5个品牌墙店铺", "小程序", "浏览5个品牌墙店铺", "浏览活动"];
// 过渡操作
...
function addMarketCar() {
    if (textContains('当前页点击浏览4个').exists() || textContains('当前页浏览加购').exists()) {
        ...
        for (index = 0; index < productList.length; index++) {
            ...
            var b = productList[index].parent().parent().children()[4].bounds()
            if (randomClick(b.centerX(),b.centerY())) {
                // 重置计时
                JUDGE_TIME = 0;
...
function interactionGrassPlanting() {
    var count = 0;
    while (true) {
        var b = className('android.view.View').indexInParent(4).depth(14).findOne().bounds()
        if (randomClick(b.centerX(), b.centerY())) {
            // 重置计时
            JUDGE_TIME = 0;
            console.info("去逛逛");
...
function getNeedSelector() {
    ...
    try{
        ...
        // if(AppType == 0 || AppType == 2)
        {
            ...
            for (let index = allSelector.length - 2; index > 0; index--) {
                for (var i = 0; i < TASK_LIST.length; i++) {
                    // if(AppType == 0)
                    // {
